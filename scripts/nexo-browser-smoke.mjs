...
async function runProfile(browser,name,viewport){
  ...
  const referenceUiTest=await page.evaluate(async()=>{
    ...
    const desktopCrown=document.querySelector('.nrx-desktop-crown');
    ...
    const desktopCrownVisible=Boolean(desktopCrown&&getComputedStyle(desktopCrown).display!=='none'&&desktopCrown.getBoundingClientRect().width>0);
  ...
  if(name!=='mobile'&&!referenceUiTest.desktopCrownVisible)failures.push(name+': botão da coroa não ficou visível no cabeçalho desktop');
  ...
  markStage('v15-interface');
  const v15InterfaceTest=await page.evaluate(async()=>{
    const app=document.querySelector('#app'),auth=document.querySelector('#authScreen');
    const pages=[...document.querySelectorAll('.page')];
    const prev={app:app?.className||'',auth:auth?.className||'',active:pages.find(p=>p.classList.contains('active'))?.id||'inicio'};
    if(app)app.classList.remove('hidden');
    ...
    const sideSvg=document.querySelectorAll('.nrx-side-item>span svg.v15-icon').length;
    const mobileSvg=document.querySelectorAll('.nrx-mob-action i svg.v15-icon').length;
    const bottomSvg=document.querySelectorAll('.nrx-bottom-nav span svg.v15-icon').length;
    const search=document.querySelector('#globalSearch');
    const searchOk=search?.placeholder==='Pesquisar no NEXO...';
