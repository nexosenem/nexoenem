...
(function(){
  const reps=[
    ['Constituição Federal de 1988','Direitos, deveres, cidadania e políticas públicas. Use o princípio apenas quando houver relação direta com o argumento.'],
    ['Declaração Universal dos Direitos Humanos','Dignidade, igualdade, liberdade, educação e proteção social. Explique a pertinência ao tema.'],
    ...
    ['Determinantes sociais da saúde','Renda, moradia, educação, saneamento e acesso a serviços como fatores de resultados em saúde.'],
    ['Sociedade em rede','Em temas digitais, relacione acesso, informação, comportamento, trabalho e desigualdade sem tratar tecnologia como causa única.']
  ];
  ...
  function mount(){
    ...
    el.innerHTML='<div class="v13-head"><div><span>BANCO DE REPERTÓRIO</span><h3>Repertório para argumentar — não para decorar</h3><p>Escolha uma referência e conecte-a à sua própria tese.</p></div></div><div class="v13-rep-grid">'+reps.map((x,i)=>'<article><b>'+v13E(x[0])+'</b><p>'+v13E(x[1])+'</p><button data-v13-rep="'+i+'">Usar no organizador →</button></article>').join('')+'</div>';
    page.appendChild(el);
    el.querySelectorAll('[data-v13-rep]').forEach(btn=>btn.onclick=()=>openBuilder(reps[Number(btn.dataset.v13Rep)][0]));
  }
