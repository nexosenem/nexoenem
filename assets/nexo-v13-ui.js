...
function v13RenderMastery(){
  ...
  root.innerHTML='<div class="v13-head"><div><span>MAPA DE DOMÍNIO</span><h3>O que está consolidado e o que precisa voltar</h3><p>Domínio combina acerto, volume, recência e comportamento de resposta.</p></div></div><div class="v13-mastery-grid">'+(rows.length?rows.map(x=>{const m=Math.round(Number(x.mastery_score||0)),status=m>=75?'Dominado':m>=55?'Consolidando':m>=35?'Aprendendo':'Prioridade';return '<article><div><span>'+v13E(x.subject||x.area)+'</span><b>'+v13E(x.topic||'Geral')+'</b></div><strong>'+m+'%</strong><div class="v13-track"><i style="width:'+m+'%"></i></div><small>'+status+' · '+Number(x.attempts||0)+' respostas</small><button data-v13-topic="'+v13E(x.topic||'')+'" data-v13-subject="'+v13E(x.subject||'')+'" data-v13-area="'+v13E(x.area||'')+'">Treinar →</button></article>'}).join(''):'<p>Resolva mais questões para construir seu mapa de domínio.</p>')+'</div>';
