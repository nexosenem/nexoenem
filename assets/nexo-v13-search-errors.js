...
async function v13Search(term){
  const q=String(term||'').trim(); if(q.length<2)return {questions:[],materials:[],videos:[]};
...
function v13OpenSearch(){
  ...
  const input=document.querySelector('#v13SearchInput'),out=document.querySelector('#v13SearchResults'); let timer;
  input.focus();
  input.oninput=function(){clearTimeout(timer);timer=setTimeout(async function(){
    const q=input.value.trim(); if(q.length<2){out.innerHTML='<div class="v13-empty">Digite pelo menos 2 caracteres.</div>';return}
    ...
    out.querySelectorAll('[data-v13-q]').forEach(function(btn){btn.onclick=async function(){const res=await client.from('questions').select('*').eq('id',Number(btn.dataset.v13Q)).maybeSingle();const data=res.data;if(!data)return;v13Close();openPage('questoes');state.current=data;state.selectedOption=null;state.answered=false;state.questionStartedAt=Date.now();await renderQuestion(data);document.querySelector('#studyWorkspace')?.classList.remove('hidden');document.querySelector('#sessionSetup')?.classList.add('hidden')}});
...
async function v13LoadErrors(){
  const root=document.querySelector('#v13Errors'); if(!root||!state.user?.id)return;
  const [a,r]=await Promise.all([
    client.from('question_attempts').select('question_id,created_at,question:questions(id,area,subject,topic,prompt)').eq('is_correct',false).order('created_at',{ascending:false}).limit(12),
    client.from('nexo_attempt_reflections').select('question_id,confidence,error_reason,created_at').order('created_at',{ascending:false}).limit(60)
  ]);
  ...
  root.innerHTML='<div class="v13-section-head"><div><span>CADERNO DE ERROS</span><h3>Erros viram próximas ações</h3></div><small>revisão ativa</small></div><div class="v13-error-list">'+rows.map(function(x){const ref=reflections.find(function(y){return Number(y.question_id)===Number(x.question_id)});return '<article><div><span>'+v13E(x.question?.subject||x.question?.area||'Questão')+'</span><b>'+v13E(x.question?.topic||'Revisão')+'</b><p>'+v13E(String(x.question?.prompt||'').slice(0,160))+'</p><small>'+(ref?(v13E(labels[ref.error_reason]||ref.error_reason||'causa não marcada')+' · '+v13E(labels[ref.confidence]||ref.confidence||'')):'causa ainda não marcada')+'</small></div><button data-v13-retry="'+x.question_id+'">Treinar tema →</button></article>'}).join('')+'</div>';
  root.querySelectorAll('[data-v13-retry]').forEach(function(btn){btn.onclick=async function(){const row=rows.find(function(x){return Number(x.question_id)===Number(btn.dataset.v13Retry)});if(!row?.question)return;openPage('questoes');await startStudySession({mode:'error_review',area:row.question.area||'',subject:row.question.subject||'',topic:row.question.topic||'',difficulty:'',visualOnly:false,size:5})}});
}
(function(){
  function add(){
    if(!document.querySelector('#v13SearchButton')){const b=document.createElement('button');b.id='v13SearchButton';b.className='v13-search-button';b.type='button';b.innerHTML='<span>⌕</span><b>Buscar</b><small>Ctrl K</small>';b.onclick=v13OpenSearch;document.body.appendChild(b)}
    const perf=document.querySelector('#desempenho');if(perf&&!perf.querySelector('#v13Errors')){const e=document.createElement('section');e.id='v13Errors';e.className='v13-panel';perf.appendChild(e)}
  }
  add();setTimeout(add,1000);
  document.addEventListener('keydown',function(e){if((e.ctrlKey||e.metaKey)&&e.key.toLowerCase()==='k'){e.preventDefault();v13OpenSearch()}});
  let u='';setInterval(function(){const id=state?.user?.id||'';if(id&&id!==u){u=id;v13LoadErrors()}if(!id)u=''},1200);
})();